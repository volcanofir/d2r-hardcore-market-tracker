let DATA=[],kind='rune',runeCollapsed=true;
...
function render(){
  ...
  if(kind==='rune'){
    ...
    cards.innerHTML=rows.length?rows.map(x=>`<article class="card item-card"><h2>${x.label}</h2><div class="fair">${fmt(x.fair_fg)} <span class="unit">FG</span></div><div class="meta"><span>樣本 ${x.samples||0}</span><span>可信度 ${x.confidence||'—'}</span></div></article>`).join(''):'<p>目前沒有自訂裝備資料。</p>'
  }
}
